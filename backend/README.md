...
* Supports backend search with `search` and a single `search_field`, so Alarm Explorer searches the backend result set instead of only the current page.
* Search fields: `alarm_id`, `device_id`, `device_name`, `device_type`, `error_code`, `error_name`, `severity`, `status`, `description`, and `raw_log`.
* Metadata search fields (`device_name`, `device_type`, `error_name`) are resolved through PostgreSQL first, then applied to ClickHouse as device/error filters.

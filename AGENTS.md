...
* **Offset-based Pagination:** Sử dụng phân trang dựa trên `offset` và `limit` để phân trang (e.g. `LIMIT :limit OFFSET :offset`).
...
* Phân trang qua Offset/Limit Pagination hoạt động mượt mà.
* Không sử dụng ORM, không dùng `SELECT *`.
